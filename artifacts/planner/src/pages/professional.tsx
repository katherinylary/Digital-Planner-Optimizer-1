import { useState } from "react";import { format } from "date-fns";import { ptBR } from "date-fns/locale";import { useProfessional } from "@/hooks/use-professional";import type { ProfessionalActivity } from "@/hooks/use-professional";import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";import { Button } from "@/components/ui/button";import { Input } from "@/components/ui/input";import { Textarea } from "@/components/ui/textarea";import { Checkbox } from "@/components/ui/checkbox";import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";import { Badge } from "@/components/ui/badge";import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";import { Briefcase, Plus, Trash2, FileText, Bell, CalendarDays } from "lucide-react";import { cn } from "@/lib/utils";

const PRIORITY_COLORS: Record<string, string> = {low: "bg-green-100 text-green-700", medium: "bg-yellow-100 text-yellow-700",high: "bg-orange-100 text-orange-700", urgent: "bg-red-100 text-red-700",};const PRIORITY_LABELS: Record<string, string> = { low: "Baixa", medium: "Média", high: "Alta", urgent: "Urgente" };

export default function Professional() {const { activities, reminders, labels, addActivity, toggleActivity, deleteActivity, saveReport, getReportForDate, addReminder, deleteReminder, addLabel } = useProfessional();const today = format(new Date(), "yyyy-MM-dd");const [actOpen, setActOpen] = useState(false);const [remOpen, setRemOpen] = useState(false);const [labelOpen, setLabelOpen] = useState(false);const [newLabel, setNewLabel] = useState("");const [reportContent, setReportContent] = useState(getReportForDate(today)?.content || "");const [actForm, setActForm] = useState({ title: "", priority: "medium" as ProfessionalActivity["priority"], labels: [] as string[], dueDate: "" });const [remForm, setRemForm] = useState({ title: "", date: "", description: "" });

const handleAddActivity = () => {if (!actForm.title.trim()) return;addActivity(actForm);setActForm({ title: "", priority: "medium", labels: [], dueDate: "" });setActOpen(false);};

const handleAddReminder = () => {if (!remForm.title.trim()) return;addReminder(remForm);setRemForm({ title: "", date: "", description: "" });setRemOpen(false);};

const handleSaveReport = () => { saveReport(today, reportContent); };

const pending = activities.filter((a) => !a.completed);const completed = activities.filter((a) => a.completed);

return (<div className="space-y-6 pb-20"><h1 className="text-2xl font-serif italic text-primary flex items-center gap-2" data-testid="text-page-title"><Briefcase className="h-6 w-6" /> Profissional</h1>
