...
export default function Schedule() {
  ...
  const [editForm, setEditForm] = useState({ title: "", date: "", time: "", endTime: "", description: "", category: "" });
...
const handleAdd = () => {
  ...
  const openEdit = (ev: Event) => {
    setEditingEvent(ev);
    setEditForm({ title: ev.title, date: ev.date, time: ev.time, endTime: ev.endTime || "", description: ev.description || "", category: ev.category });
    setEditOpen(true);
  };

  const handleEdit = () => {
    if (!editingEvent || !editForm.title.trim()) return;
    updateEvent(editingEvent.id, { title: editForm.title, date: editForm.date, time: editForm.time, endTime: editForm.endTime, description: editForm.description, category: editForm.category });
    setEditOpen(false);
...
/>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle className="font-serif italic">Editar Evento</DialogTitle></DialogHeader>
          <div className="space-y-3">
            <Input placeholder="Título do evento" value={editForm.title} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })} data-testid="input-edit-event-title" />
            <Input type="date" value={editForm.date} onChange={(e) => setEditForm({ ...editForm, date: e.target.value })} data-testid="input-edit-event-date" />
            <div className="flex gap-2">
              <div className="flex-1">
                <label className="text-xs text-muted-foreground mb-1 block">Início</label>
                <Input type="time" value={editForm.time} onChange={(e) => setEditForm({ ...editForm, time: e.target.value })} data-testid="input-edit-event-time" />
              </div>
              <div className="flex-1">
                <label className="text-xs text-muted-foreground mb-1 block">Fim</label>
                <Input type="time" value={editForm.endTime} onChange={(e) => setEditForm({ ...editForm, endTime: e.target.value })} data-testid="input-edit-event-end-time" />
              </div>
            </div>
            <Select value={editForm.category} onValueChange={(v) => setEditForm({ ...editForm, category: v })}>
              <SelectTrigger data-testid="select-edit-event-category"><SelectValue /></SelectTrigger>
              <SelectContent>
                {CATEGORIES.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}
              </SelectContent>
            </Select>
            <Textarea placeholder="Descrição (opcional)" value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} data-testid="input-edit-event-description" />
            <Button onClick={handleEdit} className="w-full" data-testid="button-save-edit-event">Salvar alterações</Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={postponeOpen} onOpenChange={setPostponeOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle className="font-serif italic flex items-center gap-2"><CalendarClock className="h-5 w-5" /> Adiar Evento</DialogTitle></DialogHeader>
          {postponingEvent && (
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    className="flex-1"
                  />
                  <Button onClick={() => handlePostpone(postponeCustomDate)} data-testid="button-postpone-custom">OK</Button>
                </div>
              </div>
            </div>
          )}
